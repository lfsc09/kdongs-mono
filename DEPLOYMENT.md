...
# Review the script
cat vps-bootstrap.sh

# Make executable and run
chmod +x vps-bootstrap.sh
sudo ./vps-bootstrap.sh your-domain.com your-email@example.com
```

### Post-Bootstrap Setup

After the bootstrap completes, you'll need to:

1. **Setup SSH Key for GitHub** (if cloning failed):
```bash
# Switch to kdongs user
su - kdongs
...
# Then re-run bootstrap to clone repository
```

2. **Verify Setup**:
```bash
# Check directory structure
ls -la /var/www/kdongs-mono

# Verify secrets exist
ls -la /var/www/kdongs-mono/docker/secrets/

# Verify backend .env exists
ls -la /var/www/kdongs-mono/packages/backend/.env
