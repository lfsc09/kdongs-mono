...
} from '@kdongs-mono/domain/dto/investment/analytic/liquidation-series-dto'
import { PerformanceAnalyticsResponse } from '@kdongs-mono/domain/dto/investment/analytic/performance-dto'
...
export default class AnalyticService {
  ...
  async performance(
    input: Awaited<ReturnType<typeof performanceValidator.validate>>,
  ): Promise<AdonisJSResponse<PerformanceAnalyticsResponse> | null> {
  ...
  async liquidationSeries(
    input: Awaited<ReturnType<typeof liquidationSeriesValidator.validate>>,
  ): Promise<AdonisJSResponse<LiquidationSeriesAnalyticsResponse>> {
    const isWalletIdsArray = Array.isArray(input.walletIds)
    const wallets = await Wallet.query()
      .if(
        !input.walletIds || (isWalletIdsArray && input.walletIds.length === 0),
