...
@inject()
export default class WalletMovementService {
  constructor(protected logger: Logger) {}

  async list(
    input: Awaited<ReturnType<typeof indexValidator.validate>>,
  ): Promise<AdonisJSPaginationResponse<IndexWalletMovementResponse>> {
  ...
  async create(): Promise<AdonisJSResponse<CreateWalletMovementResponse>> {
    return {
      data: {
  ...
  ): Promise<AdonisJSResponse<ShowWalletMovementResponse>> {
    const wallet = await Wallet.query()
      .where('id', input.walletId)
      .where('userId', input.userId)
      .firstOrFail()
  ...
  ): Promise<AdonisJSResponse<EditWalletMovementResponse>> {
    const wallet = await Wallet.query()
      .where('id', input.walletId)
      .where('userId', input.userId)
      .firstOrFail()
  ...
  async hardDelete(input: Awaited<ReturnType<typeof deleteValidator.validate>>): Promise<void> {
    const wallet = await Wallet.query()
      .where('id', input.walletId)
      .where('userId', input.userId)
      .firstOrFail()
