    ...
    const dailyRate =
      interestType === 'fixed'
        ...
        : getDailyVariablePerformance(
            indexValue,
            new Big(faker.number.float({ fractionDigits: 2, max: 0.15, min: 0.05 })),
            true,
          )
...
export const AssetBrlPrivateBondFactory = factory
  .define(AssetBrlPrivateBond, async ({ faker }) => {
    ...
    const maturityDateUtc = isDone
      ? faker.date.recent({ days: faker.number.int({ max: 365, min: 30 }) })
      : faker.date.soon({ days: faker.number.int({ max: 365, min: 30 }) })
    const enterDateUtc = getEnterDateUtc(maturityDateUtc, faker)
    const inputAmount = new Big(faker.number.float({ fractionDigits: 2, max: 200000, min: 1000 }))
  ...
  .state('asDone', (b, ctx) => {
    ...
    const maturityDateUtc = ctx.faker.date.recent({
      days: ctx.faker.number.int({ max: 365, min: 30 }),
    })
