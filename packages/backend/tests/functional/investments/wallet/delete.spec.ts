...
test.group('[delete] user wallet', group => {
   ...
   * ACCESS TESTS
   */
  test('should not be able to delete user wallet [no token]', async ({ client }) => {
    const output = await client.delete(`investments/wallets/${uuidv7()}`)
    output.assertStatus(401)
  })

  test("should not be able to delete user wallet [unaccepted '{$self}' role token]")
  ...
  test("should be able to delete user wallet [accepted '{$self}' role token]")
    ...
    .run(async ({ client }, role) => {
      ...
      const output = await client
        ...
        .withGuard('api')
        .loginAs(user, userTokenAbilities(role))
      output.assertStatus(204)
    })

  test("should not be able to delete another user's wallets [not owner]", async ({ client }) => {
   ...
   * RETURN TEST
   */
  test('should be able to delete own wallet [owner of wallet]', async ({ client }) => {
    const user = await UserFactory.merge({ role: UserRoles.user }).create()
    const wallet = await WalletFactory.merge({ userId: user.id }).create()
    const output = await client
      .delete(`investments/wallets/${wallet.id}`)
